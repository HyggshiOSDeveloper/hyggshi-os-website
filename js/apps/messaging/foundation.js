...
function gcGenerateInviteCode(length = 8) {
...
function gcGetGroupInviteCode(roomId = gcCurrentRoom) {
    const room = gcGetRoomById(roomId);
    if (!room || room.type !== 'group') return '';
    const explicitCode = String(room.invite_code || '').trim().toLowerCase();
    if (explicitCode) return explicitCode;
    const roomIdText = String(room.id || '').trim().toLowerCase();
    const parts = roomIdText.split('-').filter(Boolean);
    const suffix = parts[parts.length - 1] || '';
    if (/^[a-z0-9]{6,12}$/.test(suffix)) return suffix;
    return roomIdText;
}

function gcGetInviteRemainingJoins(roomId = gcCurrentRoom) {
    const room = gcGetRoomById(roomId);
    if (!room) return null;
    const maxJoins = Number(room.invite_max_joins);
    const joinCount = Math.max(0, Number(room.invite_join_count) || 0);
    if (!Number.isFinite(maxJoins) || maxJoins <= 0) return null;
    return Math.max(0, maxJoins - joinCount);
}

function gcIsInviteExpired(roomId = gcCurrentRoom) {
    const room = gcGetRoomById(roomId);
    if (!room?.invite_expires_at) return false;
    const expiresAt = new Date(room.invite_expires_at).getTime();
    return Number.isFinite(expiresAt) && expiresAt <= Date.now();
}

function gcIsInviteJoinLimitReached(roomId = gcCurrentRoom) {
    const remaining = gcGetInviteRemainingJoins(roomId);
    return remaining !== null && remaining <= 0;
}

function gcGetInviteExpiryLabel(roomId = gcCurrentRoom) {
    const room = gcGetRoomById(roomId);
    if (!room?.invite_expires_at) return 'Never';
    return new Date(room.invite_expires_at).toLocaleString();
}

function gcBuildGroupInviteLink(roomId = gcCurrentRoom) {
    const room = gcGetRoomById(roomId);
    if (!room || room.type !== 'group') return '';
    const inviteCode = gcGetGroupInviteCode(roomId);
    if (!inviteCode) return '';

    try {
        const url = new URL(window.location.href);
        url.searchParams.set('gc_join', inviteCode);
        return url.toString();
    } catch (error) {
        const base = window.location.href.split('?')[0];
        return `${base}?gc_join=${encodeURIComponent(inviteCode)}`;
    }
}

function gcGetInviteTokenFromUrl() {
    try {
        const url = new URL(window.location.href);
...
function gcResolveInviteRoom(inviteToken) {
    const token = String(inviteToken || '').trim().toLowerCase();
    if (!token) return null;

    const directRoom = gcGetRoomById(token);
    if (directRoom?.type === 'group') return directRoom;

    return gcRoomCache.find(room => {
        if (!room || room.type !== 'group') return false;
        return gcGetGroupInviteCode(room.id) === token;
    }) || null;
}
...
async function gcHandleInviteLink() {
    const inviteToken = gcGetInviteTokenFromUrl();
    if (!inviteToken || !sbClient || !gcUserId) return false;

    try {
        const room = gcResolveInviteRoom(inviteToken);
        if (!room || room.type !== 'group') {
            gcClearInviteRoomIdFromUrl();
            gcNotifyError('This invite link is invalid or the group no longer exists.');
            return false;
        }

        const { data: existingMembership } = await sbClient
        ...
        if (!existingMembership) {
            if (gcIsInviteExpired(room.id)) {
                gcClearInviteRoomIdFromUrl();
                gcNotifyError('This invite link has expired.');
                return false;
            }
            if (gcIsInviteJoinLimitReached(room.id)) {
                gcClearInviteRoomIdFromUrl();
                gcNotifyError('This invite link has reached its join limit.');
                return false;
            }
        }
        ...
        if (!existingMembership) {
            const nextJoinCount = (Number(room.invite_join_count) || 0) + 1;
            const { error: updateInviteError } = await sbClient
                .from(GC_TABLES.rooms)
                .update({ invite_join_count: nextJoinCount })
                .eq('id', room.id);

            if (!updateInviteError) {
                room.invite_join_count = nextJoinCount;
                gcRoomCache = gcRoomCache.map(item => item.id === room.id ? { ...item, invite_join_count: nextJoinCount } : item);
            }
        }

        gcMarkGroupJoined(room.id);
        gcClearInviteRoomIdFromUrl();
        await gcSwitchRoom(room.id);
        showNotification(
            'Zashi Messaging',
            existingMembership
                ? `Opened group "${room.name}". You are already a member.`
                : `Joined group "${room.name}".`
        );
        return true;
    } catch (error) {
